...
struct CustomShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        return Path { path in
            ...
            path.addArc(center: CGPoint(x: rect.maxX, y: rect.maxY - cutoutModifier), radius: rect.height/20, startAngle: Angle(degrees: 90), endAngle: Angle(degrees: 270), clockwise: false)
